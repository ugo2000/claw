...
async function doImport() {
  ...
  try {
    ...
    importResult.value = `Imported ${success} client(s). Failed: ${failed}`
...
function mapRow(row) {
  ...
  const nameKey = find('company', 'name', 'company name', 'company_name')
  const emailKey = find('email', 'mail', 'email address')
  const phoneKey = find('phone', 'telephone', 'mobile', 'tel')
  const contactKey = find('contact', 'contact person', 'contact_name')
  const countryKey = find('country', 'nation')
  const noteKey = find('notes', 'note', 'remark', 'comment')
  const websiteKey = find('website', 'web', 'site', 'url')
...
// URL / Email validation helpers for template display
